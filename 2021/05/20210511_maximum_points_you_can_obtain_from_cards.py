...
class Solution:
    def maxScore(self, cardPoints: List[int], k: int) -> int:
        ...
        n = len(cardPoints)
        window = n - k
        current_sum = sum(cardPoints[:window])
        answer = total_pts - current_sum
        for i in range(window, n):
            current_sum = current_sum - cardPoints[i - window] + cardPoints[i]
            answer = max(answer, total_pts - current_sum)
